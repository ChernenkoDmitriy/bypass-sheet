export const translations = {
  en: {
    theme: 'Theme',
    successCreateCompany: 'The company is successfully created!',
    companyName: 'Company name',
    newCompany: 'New company',
    refresh: 'Refresh',
    noConnection: 'Internet problems',
    createCompany: 'Create a company',
    connectToCompany: 'Join the company',
    successfulRegistration: 'Successful registration!',
    errorToast: 'Error',
    userNotFound: 'User not found!',
    firstName: 'First name',
    lastName: 'Last name',
    signIn: 'Sign in',
    fullNameError: 'Enter your full name',
    phoneError: "Wrong number.",
    restore: 'Restore',
    resetPassword: 'Reset password',
    policy: 'By creating an account, you agree to the Terms of Use and Privacy Policy of Company standart.',
    createAnAccount: 'Create an account',
    fullName: 'Full name (required)',
    workPhoneNumber: 'Work phone number',
    personalization: 'We will use this to personalize the account',
    createProfile: 'Create profile',
    registration: 'Registration',
    forgottenPassword: 'Forgotten password ?',
    addPhoneNumber: "Enter the number",
    addPassword: "Enter the password",
    password: "Password",
    phoneNumber: 'Phone number',
    deleteObject: 'Delete object',
    editObject: 'Edit object',
    googleSheetInstruction: '* If the table id and name were not specified, then a table named CompanyStandardsApp will be created in your Google Drive',
    addPhoto: 'Add photo',
    newPhotoDescription: 'Take a new photo or upload from gallery',
    uploadPhoto: 'Upload Photo',
    takePhoto: 'Take photo',
    continue: 'Continue',
    photo: 'Photo',
    criterialObject: 'Evaluation criteria',
    commonInformation: 'Common Information',
    objectOfAssessment: 'Assessment',
    bypassList: 'Bypass lists',
    enterAddress: 'Enter address...',
    address: 'Object address',
    groupName: 'Object name',
    language: 'Language',
    languages: 'Languages',
    ru: 'Русский',
    en: 'English',
    ua: 'Українська',
    sheetName: 'Sheet name',
    googleSheet: 'Google sheet ID',
    localReport: 'Local report',
    cancel: 'Cancel',
    ok: 'Ok',
    signOutAccountText: 'Are you sure you want to log out of your account?',
    signOut: 'Sign out',
    newObject: 'New object',
    objectsOfAssessment: 'Objects of assessment',
    update: 'Update',
    addSubtask: 'Add subtask',
    delete: 'Delete',
    edit: 'Edit',
    addBypassSheet: 'Add bypass sheet',
    enterGroupName: 'Enter group name',
    createGroup: 'Create group',
    addComment: 'Add comment',
    comment: 'Comment',
    taskDescribe: 'Enter task describe',
    createBypassSheet: 'Bypass sheet',
    addItem: 'Add item',
    create: 'Create',
    createGoogleReport: 'Create in Google table',
    title: 'Title',
    emptyBypassSheetList: "You don't have bypass sheets. You can create a group and add bypasses to sheets",
    appName: 'Company standards',
    settings: 'Settings',
    dashboard: 'Dashboard',
    company: 'My company',
    profile: 'Profile',
    english: 'English',
    ukrainian: 'Ukrainian',
    dark: 'Dark',
    light: 'Light',
    employees: 'Employees',
    attention: 'Attention: ',
    geolocationMustBeEnabled: 'Geolocation must be enabled for the app to work.',
    requestPermission: 'Request Permission',
    suchUserAlreadyExists: 'Such a user already exists',
    changeCompany: 'Change company',
    tabulation: 'Tabulation',
    workShift: 'Work shift',
    users: 'Users',
    workShiftEmptyList: 'You can create a work shift template',
    createWorkShift: 'Create work shift',
    name: 'Name',
    timeStart: 'Time start',
    timeEnd: 'Time end',
    addUser: 'Add user',
    editWorkShift: 'Edit work shift',
    searchPlaceholder: 'Search by name',
    doDelete: 'Do you really want to delete?',
    timeInputError: 'Be sure to choose a date!',
    newName: 'New name',
    invite: 'Invite: ',
    accept: 'Accept',
    reject: 'Reject',
    role: 'Your role : ',
    isAdmin: 'Admin',
    isSuccessInvite: 'The invitation is sent!',
    members: 'Members',
    requestSent: 'Invite',
    schedule: 'Schedule',
    logOut: 'Sing out',
    passwordError: 'It can\'t be empty',
    addAddress: "Add address",
    selectLocationWorkPlace: 'Select a point on the map',
    workPlaceEmptyList: 'You can add an address',
    listAddress: 'Address list',
    updateAddress: 'Update address',
    selectedAddress: 'Selected address',
    January: 'January',
    February: 'February',
    March: 'March',
    April: 'April',
    May: 'May',
    June: 'June',
    July: 'July',
    August: 'August',
    September: 'September',
    October: 'October',
    November: 'November',
    December: 'December',
    Jan: 'Jan',
    Feb: 'Feb',
    Mar: 'Mar',
    Apr: 'Apr',
    Jun: 'Jun',
    Jul: 'Jul',
    Aug: 'Aug',
    Sep: 'Sep',
    Oct: 'Oct',
    Nov: 'Nov',
    Dec: 'Dec',
    Sun: 'Sun',
    Mon: 'Mon',
    Tue: 'Tue',
    Wed: 'Wed',
    Thu: 'Thu',
    Fri: 'Fri',
    Sat: 'Sat',
    save: 'Save',
    comparePeriod: 'Compare to previous period',
    history: 'History',
    start: 'Start',
    end: 'End',
    openWorkShift: 'Open a work shift',
    closeWorkShift: 'Close the work shift',
    noneData: 'No data',
    toStartShift:'To start a shift, click the button above',
  },
  uk: {
    toStartShift:'Щоб почати робочу зміну , натисніть кнопку вище',
    noneData: 'Немає даних',
    openWorkShift: 'Відкрити робочу зміну',
    closeWorkShift: 'Закрити робочу зміну',
    start: 'Початок',
    end: 'Кінець',
    history: 'Історія',
    comparePeriod: "Порівняти з попереднім періодом",
    save: 'Зберегнти',
    January: 'Січень',
    February: 'Лютий',
    March: 'Березень',
    April: 'Квітень',
    May: 'Травень',
    June: 'Червень',
    July: 'Липень',
    August: 'Серпень',
    September: 'Вересень',
    October: 'Жовтень',
    November: 'Листопад',
    December: 'Грудень',
    Jan: 'Січ',
    Feb: 'Лют',
    Mar: 'Бер',
    Apr: 'Квіт',
    Jun: 'Черв',
    Jul: 'Лип',
    Aug: 'Серп',
    Sep: 'Вер',
    Oct: 'Жовт',
    Nov: 'Лист',
    Dec: 'Груд',
    Sun: 'Нд',
    Mon: 'Пн',
    Tue: 'Вт',
    Wed: 'Ср',
    Thu: 'Чт',
    Fri: 'Пт',
    Sat: 'Сб',
    selectedAddress: 'Вибрана адреса',
    updateAddress: 'Змінити адресу',
    listAddress: 'Список адрес',
    workPlaceEmptyList: 'Ви можете додати адресу',
    selectLocationWorkPlace: 'Оберіть точку на карті',
    addAddress: "Додати адресу",
    logOut: 'Вийти',
    schedule: 'Графік',
    requestSent: 'Запит надіслано',
    members: 'Користувачі',
    isSuccessInvite: 'Запрошення відправлено!',
    role: 'Ваша роль :',
    isAdmin: 'Адмін',
    accept: 'Прийняти',
    reject: 'Відмінити',
    invite: 'Запрошення',
    newName: 'Нова назва',
    timeInputError: "Не забудьте обрати дату!",
    doDelete: 'Ви дійсно бажаете видалити?',
    searchPlaceholder: 'Пошук за ім\'ям',
    editWorkShift: 'Редагувати графік',
    addUser: 'Додати користувача',
    timeStart: 'Time start',
    timeEnd: 'Time end',
    name: 'Назва',
    createWorkShift: 'Створення графіку',
    workShiftEmptyList: 'Ви можете створити шаблон робочої зміни',
    users: 'Користувачі',
    workShift: 'Робоча зміна',
    tabulation: 'Табелювання',
    changeCompany: 'Змінити компанію',
    suchUserAlreadyExists: 'Такий користувач вже існує',
    requestPermission: 'Запитати дозвіл',
    geolocationMustBeEnabled: 'Для роботи програми має бути ввімкнена геолокація.',
    attention: 'Увага: ',
    edit: 'Редагувати',
    delete: 'Видалити',
    employees: 'Сотрудников',
    dark: 'Темна',
    light: 'Світла',
    english: 'Англійська',
    ukrainian: 'Українська',
    dashboard: 'Приборна панель',
    company: 'Мої компаниї',
    profile: 'Профіль',
    settings: 'Налаштування',
    googleSheetInstruction: '* Якщо ID та ім\'я таблиці не вказані, то буде створено таблицю з іменем CompanyStandardsApp у вашому Google Drive',
    addPhoto: 'Додати фото',
    newPhotoDescription: 'Зробіть нове фото або завантажте з галереї',
    uploadPhoto: 'Завантажити фото',
    takePhoto: 'Зробити фото',
    language: 'Мова',
    en: 'Англійська',
    ua: 'Українська',
    languages: 'Мови',
    sheetName: 'Назва таблиці',
    googleSheet: 'ID гугл таблиці',
    localReport: 'Локальний звіт',
    cancel: 'Скасувати',
    ok: 'Ок',
    signOutAccountText: 'Ви впевнені, що хочете вийти з аккаунта?',
    signOut: 'Вийти',
    editObject: 'Редагувати об\'єкт',
    criterialObject: 'Критерії оцінювання',
    deleteObject: 'Видалити об\'єкт',
    update: 'Оновити',
    criterial: 'Критерії оцінювання',
    groupName: 'Назва об\'єкту',
    enterGroupName: 'Введіть назву...',
    continue: 'Продовжити',
    photo: 'Фото',
    objectOfAssessment: 'Об\'єкт оцінювання',
    bypassList: 'Обхідні листи',
    enterAddress: 'Введіть адресу...',
    address: 'Адреса об\'єкту',
    commonInformation: 'Загальна інформація',
    newObject: 'Новий об\'єкт',
    objectsOfAssessment: 'Об\'єкти оцінювання',
    comment: 'Коментар',
    taskDescribe: 'Введіть опис завдання',
    createBypassSheet: 'Обхідний лист',
    addItem: 'Додати поле',
    create: 'Створити',
    createGoogleReport: 'Створити у Google таблиці',
    title: 'Заголовок',
    emptyBypassSheetList: 'Для додавання об\'єкту оцінювання оберіть "Новий об\'єкт" у верхньому меню',
    appName: 'Стандарти компанії',
    phoneNumber: 'Номер телефону',
    password: "Пароль",
    addPhoneNumber: "Введіть номер",
    addPassword: "Введіть пароль",
    forgottenPassword: 'Забули пароль?',
    registration: 'Реєстрація',
    createProfile: 'Створіть обліковий запис',
    personalization: 'Ми будемо використовувати це для персоналізації облікового запису.',
    workPhoneNumber: 'Робочий номер телефону',
    fullName: 'Повне ім\'я (обов\'язково)',
    createAnAccount: 'Створити обліковий запис',
    policy: 'Створюючи обліковий запис, ви погоджуєтеся з Умовами використання та Політикою конфіденційності компанії.',
    resetPassword: 'Скинути пароль',
    restore: 'Відновити',
    phoneError: "Невірний номер",
    passwordError: 'Не може бути порожнім',
    fullNameError: 'Введіть своє повне ім\'я',
    signIn: 'Увійти',
    firstName: 'Прізвище',
    lastName: 'Ім\'я',
    userNotFound: 'Користувача не знайдено!',
    errorToast: 'Помилка',
    successfulRegistration: 'Успішна реєстрація!',
    createCompany: 'Створити компанію',
    connectToCompany: 'Приєднатися до компанії',
    noConnection: 'Проблеми з інтернетом',
    refresh: 'Оновити',
    newCompany: 'Нова компанія',
    companyName: 'Назва компанії',
    successCreateCompany: 'Компанію успішно створено!',
    theme: 'Тема',
  }

};